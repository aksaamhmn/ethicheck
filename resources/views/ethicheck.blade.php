...
<head>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Pilih elemen-elemen penting pakai ID
            ...
            analysisForm.addEventListener('submit', async (e) => {
                ...
                try {
                    ...
                    const response = await fetch("{{ route('ethicheck.analyze') }}", {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken, // Ambil dari meta tag
                    ...
                    // Render hasil ke results-box
                    renderResults(data);

                } catch (error) {
                    console.error('Error:', error);
                    resultsBox.innerHTML = `<p style="color: var(--red-border);"><strong>Terjadi Kesalahan:</strong> ${error.message}</p>`;
                } finally {
            ...
            // --- FUNGSI RENDER YANG DIPERBARUI ---
            function renderResults(data) {
                // Cek jika data adalah null atau undefined
                if (!data) {
                ...
                if (data.highlighted_text) {
                    // Ganti newline (\n) dengan tag <br> agar format paragraf tetap terjaga
                    highlightedContent.innerHTML = data.highlighted_text.replace(/\n/g, '<br>');
                } else {
                    // Fallback jika AI tidak kirim 'highlighted_text' (ini yang terjadi sebelumnya)
                    highlightedContent.innerHTML = '<p style="color:var(--red-border);">(Peringatan: AI tidak mengembalikan `highlighted_text`.)</p>';
                }
                resultsBox.appendChild(highlightedContent);

                // 2. Tampilkan Penjelasan (Jika ada)
                ...
                if (data.status_message) {
                    ...
                    statusMsg.textContent = `✅ ${data.status_message}`; // "Berita sudah sesuai pedoman"
